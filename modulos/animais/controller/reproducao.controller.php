...
class ReproducaoController 
{
    ...
    public function listar_pendencias_coberturas(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        $resultado = $this->reproducao->listar_pendencias_coberturas($request);
        $response->getBody()->write($resultado);
        return $response->withStatus(json_decode($resultado)->http_status_code)->withHeader('Content-type', 'application/json');
