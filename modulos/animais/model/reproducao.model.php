...
class ReproducaoModel
{
    ...
    public function detalhes_reproducao_cobertura(ServerRequestInterface $request)
    {
        ...
        if (!@$id_proprietario || !@$id_cobricao) return erro("Cobertura ou Proprietário com identificação incorreta!");

        ( new UsuarioController() )->checa_permissao_acesso($params['id_usuario'], 7);
    ...
    public function detalhes_reproducao_nascimento(ServerRequestInterface $request)
    {
        ...
        if (!@$id_proprietario || !@$id_nascimento) return erro("Nascimento ou Proprietário com identificação incorreta!");

        ( new UsuarioController() )->checa_permissao_acesso($params['id_usuario'], 5);
    ...
    public function listar_banco_nomes(ServerRequestInterface $request)
    {
        ...
        if (!trim(@$sexo) || !trim(@$letra_alfabeto) )  return erro("Parâmetros inválidos ou faltantes!");

        ( new UsuarioController() )->checa_permissao_acesso($params['id_usuario'], 8);
    ...
    public function listar_coberturas(ServerRequestInterface $request)
    {
        ...
        if (!@$id_proprietario || !@$tipo_gestacao || !@$situacao_nascimento || !@$situacao_prenhez) return erro("Parâmetros inválidos ou faltantes!");
    ...
    public function listar_nascimentos(ServerRequestInterface $request)
    {
        ...
        if (!@$id_proprietario || !@$situacao_produto) return erro( "Parâmetros inválidos ou faltantes!");
    ...
    public function listar_programacao_monta(ServerRequestInterface $request)
    {
        ...
        if (!@$id_proprietario || !@$metodo_reprodutivo || !isset($estacao)) return erro("Parâmetros inválidos ou faltantes!");

        ( new UsuarioController() )->checa_permissao_acesso($params['id_usuario'], 6);
    ...
    public function listar_pendencias_coberturas(ServerRequestInterface $request)
    {
        $params = (array)$request->getParsedBody();
        $id_proprietario     =  @$params['id_proprietario'];
        ...
        try {
            $query_sql = 
                        ...
                        tab_cobricoes.id_cobricao as ID_COBRICAO, 
                        DATE_FORMAT(tab_cobricoes.data_cobertura, '%d/%m/%Y') as DATA_COBRICAO, 
                        DATEDIFF(NOW(), tab_cobricoes.data_cobertura) AS NUMERO_DIAS_COBRICAO,
                        CONCAT(tab_garanhao.nome, ' x ', tab_doadora.nome) as GARANHAO_DOADORA_COBRICAO, 
                        IF(tab_cobricoes.id_te = '18', DATE_FORMAT(tab_cobricoes.data_te, '%d/%m/%Y'), '-') as DATA_TE_COBRICAO, 
                        IF(ISNULL(tab_receptora.nome),'-',CONCAT(tab_receptora.marca, ' - ', tab_receptora.nome)) as NOME_RECEPTORA_COBRICAO, 
                        (CASE WHEN tab_toques.id_situacao_prenhez IS NULL THEN 'SEM TOQUE' ELSE UPPER(tab_situacao_toque.descricao) END) as TOQUE_COBRICAO, 
                    ...
                    WHERE
                        ...
                        tab_comunicacoes_cobricao_associacao.protocolo_comunicacao IS NULL AND
                        tab_toques.id_situacao_prenhez = 19
                    GROUP BY tab_cobricoes.id_cobricao
